...
public class GLText {
  ...
  public final static int CHAR_CNT = (((CHAR_END - CHAR_START) + 1)
      + 1);  // Character Count (Including Character to use for Unknown)
  ...
  //--Members--//
  Resources resources;                               // Asset Manager
  ...
  public GLText(Program program, Resources resources) {
    if (program == null) {
      program = new BatchTextProgram();
      program.init();
    }
    this.resources = resources;                           // Save the Asset Manager Instance
  ...
  public GLText(Resources resources) {
    this(null, resources);
  }
  ...
  public boolean load(int font, int size, int padX, int padY) {
    ...
    Typeface tf = resources.getFont(font);         // Create the Typeface from Font File
    ...
    fontHeight = (float) Math
        .ceil(Math.abs(fm.bottom) + Math.abs(fm.top));  // Calculate Font Height
    ...
    Bitmap bitmap = Bitmap
        .createBitmap(textureSize, textureSize, Bitmap.Config.ALPHA_8);  // Create Bitmap
    ...
    textureRgn = new TextureRegion(textureSize, textureSize, 0, 0, textureSize,
        textureSize);  // Create Full Texture Region
  ...
  // R: [none]
  public void draw(String text, float x, float y, float z, float angleDegX, float angleDegY,
      float angleDegZ) {
    ...
    for (int i = 0; i < len; i++) {              // FOR Each Character in String
      ...
      batch.drawSprite(letterX, letterY, chrWidth, chrHeight, charRgn[c],
          modelMatrix);  // Draw the Character
      letterX +=
          (charWidths[c] + spaceX) * scaleX;    // Advance X Position by Scaled Character Width
    }
  }
  ...
  public float drawC(String text, float x, float y, float z, float angleDegX, float angleDegY,
      float angleDegZ) {
    float len = getLength(text);                  // Get Text Length
    draw(text, x - (len / 2.0f), y - (getCharHeight() / 2.0f), z, angleDegX, angleDegY,
        angleDegZ);  // Draw Text Centered
